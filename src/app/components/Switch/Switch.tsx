...
export default function Switch() {
  ...
  return (
    <div onClick={handleClick} className='text-xs cursor-pointer bg-[#2D2D33] p-2 flex font-bold rounded-full relative text-center'>
        <div id='badge-switch' className='transition-all px-8 py-2 rounded-full bg-primary-color text-primary-color absolute whitespace-nowrap w-[180px]'>
            {/* <span className='opacity-0 whitespace-nowrap text-primary-color'>Directorio Copu</span> */}
        </div>
        <div id='off-state' className={`transition px-8 py-2 rounded-full relative w-[180px] ${on && 'hover:bg-[#3C3C45]'}`}>
            <span className={`${on ? 'text-text-light' : 'text-text-dark'}`}>Web Copu</span>
        </div>
        <div id='on-state' className={`px-8 py-2 rounded-full relative whitespace-nowrap w-[180px] transition ${!on && 'hover:bg-[#3C3C45]'}`}>
